...
const DIRECTIONS = {
  work: [
    ...
    { category: 'ai-coding', categoryName: 'AI编程',   desc: 'Cursor/Copilot/TRAE/Claude Code 新功能、AI编程最佳实践、AI与开发者关系' },
  ...
  life: [
    ...
    { category: 'current',   categoryName: '时事热点', desc: '近期热门新闻、社会话题、科技动态、引发讨论的事件' },
...
## 用户画像
- 工作：React Native 开发、App 开发、前端程序员
...
## 生成要求
1. 生成 6-8 条梗，每条包含：title（标题）、category（分类key）、categoryName（分类名）、content（梗的描述，生动有趣）、explanation（梗的解释/背景，帮用户理解）、usageTip（怎么用这个梗，具体到可以怎么开口聊）
2. 梗要有趣、接地气，能引发共鸣和笑声
3. 内容要有时效性，结合近期热点（如果有的话）
4. 解读要清晰，让不完全了解背景的人也能听懂
5. usageTip 要具体，给出可以直接用的开场白或聊天切入方式
6. category 必须是以下之一：rn, frontend, app-dev, ai-coding, devtools, edu, pingpong, current, family, life
7. 不要生成已有的重复内容，尽量有新意
